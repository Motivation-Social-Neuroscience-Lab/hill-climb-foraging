...
# difference pursue rate in poor vs rich environments correlated with reward earned
d.opp.cost <- d %>%
  filter(effortLevel == 'mid') %>%
  group_by(subjectNumber, blockType) %>%
  summarise(acceptRate = mean(response)) %>%
  ungroup()
opp_cost <- d.opp.cost$acceptRate[d.opp.cost$blockType == 'hard'] - d.opp.cost$acceptRate[d.opp.cost$blockType == 'easy']

d.reward.corr <- d %>%
  group_by(subjectNumber) %>%
  summarise(rewardTotal = sum(reward)) %>%
  mutate(opp_cost = opp_cost)

image <- ggplot2::ggplot(d.reward.corr, aes(x = opp_cost, y= rewardTotal)) +
  geom_vline(xintercept = 0, linetype = 'dashed', linewidth = 0.2) +
  geom_point(shape = 21, fill = "#B2DFDB", colour = 'white', size = figure$subj_point_size, stroke = 0.1) +
  xlab('pr(accept): hard - easy') + ylab('credits earned') +
  geom_smooth(method='lm', formula= y~x, colour = "#00695C", linewidth = 0.4) +
  theme_classic() +
  stat_cor(method = 'pearson', p.digits = 2, size = 2.5, label.x = 0.02) +
  theme(text = element_text(size = figure$font_size))

image
ggsave(file = paste0('../figures/plots/', task_version, '/', model_number, 'corr_credits_opp_cost.pdf'), plot = image, width = 6, height = 6, unit = 'cm', device = "pdf", bg = "transparent")
...
# load and extract p-values from glm object
load(file = paste0('glmm_output/glm_model_avg_eff_', model_number, task_version, '.RData'))
