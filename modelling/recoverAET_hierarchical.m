...
%% SET UP --------------------------------------------------------------
% load fit parameters
load(sprintf('../../data_derived/%s/fitting_hierarchical/fitting_hierarchical_M%d', study_version, model_id));
...
for isub = 1:nsims
    ...
    [~, ~, simdata{isub}] = simulate_AET_task_new(config.task, modout.model, agent, sim_params_real(:, isub)');

end

% Get parameter names and bounds
    [params] = buildParams_new(modout.model);
...
%         [params] = buildParams_new(tmp_model);
