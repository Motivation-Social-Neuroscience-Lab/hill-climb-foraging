...
study_version = 'v3';
nsims = 1000; % number of simulated participants (doesn't apply if using 'fit' parameters)
fit_type = 'MAP'; % MLE or MAP fitting
param_type = 'fit'; % which parameters to simulate {uniform, fit, median, input}

% specify parameters if inputting 
discount = 1;
beta = 5;
alpha = 0.1;
...
switch param_type
    ...
    case 'input'
        params = repmat([discount; beta; alpha], [1,nsims]);
end
...
if save_data == 1% save simulations for further analysis in R
    writetable(out,[config.paths.data_behav, sprintf('behav_summary_M%d_%s.csv', modelNum, study_version)]);
