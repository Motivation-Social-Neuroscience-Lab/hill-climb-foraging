...
%% SET UP --------------------------------------------------------------
% load fit parameters
load(sprintf('../../data_derived/%s/fitting_hierarchical/fitting_hierarchical_M%d', study_version, modelNum));
...
for isub = 1:nsims
    ...
    [~, simdata{isub}] = simulate_AET_task_new(config.task, modout.model, agent, params(:, isub)');
...
if save_data == 1% save simulations for further analysis in R
    writetable(out,[config.paths.dataDerived, sprintf('cleaned_behav_summary_M%d_%s.csv', modelNum, study_version)]);
    param_table = array2table(modout.fitted_params_real', 'VariableNames',modout.param_names);
    writetable(param_table,[config.paths.dataDerived, sprintf('fitting_hierarchical/fit_params_M%d.csv', modelNum)]);
end
