...
for i = 1:length(matFiles)
    ...
    %% FOR TMFC (gPPI) - do not include the miss regressor, as each subject needs the same condition order for the design matrix
        % names{7} = 'miss'; % create the names for each condition
        % onsets{7} = []; % no onsets unless they missed trials
    ...
    save_onsets_dir = '/Volumes/appsmaj-effort-prey-fmri-scholey/aet_fMRI/first_level/z_6m_csf_wm_compcor_M1_fit_MAP/glm2_1_TMFC_v3/sub-';
