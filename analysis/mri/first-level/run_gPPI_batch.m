%% Script to run TMFC analyses
% https://github.com/Masharipov/TMFC_toolbox/blob/main/examples/TMFC_command_window_example.m
% 24 March 2026
...
cd '/Volumes/appsmaj-effort-prey-fmri-scholey/aet_fMRI/gPPI_analysis'

% The path where all results will be saved
tmfc.project_path = pwd;
...
if plot_only == 0
    ...
    addpath '/Volumes/appsmaj-effort-prey-fmri-scholey/spm'
    % 2) Set path to TMFC_toolbox (Add with subfolders)
    addpath(genpath('/Volumes/appsmaj-effort-prey-fmri-scholey/toolboxes/TMFC_toolbox/'))
    ...
    for i = 1:numel(subj)
        ...
        tmfc.subjects(i).path = ['/Volumes/appsmaj-effort-prey-fmri-scholey/aet_fMRI/first_level/z_6m_csf_wm_compcor_M1_fit_MAP/glm2_1_TMFC_v3/sub-', id, '/SPM.mat'];
    end
    ...
    save(fullfile(tmfc.project_path,'TMFC_AET.mat'),'tmfc');
    ...
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(1).name = 'cue'; %(see SPM.Sess.U.name(kPmod))
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(1).file_name = '[Sess_1]_[Cond_1]_[cue]';
    ...
    % First modulator for first condition:
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(2).sess   = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(2).number = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(2).pmod   = 2;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(2).name = 'cuexvalue^1';
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(2).file_name = '[Sess_1]_[Cond_1]_[cuexvalue^1]';
    % Second modulator for first condition:
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(3).sess   = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(3).number = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(3).pmod = 3;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(3).name = 'cuexbackgroundEffort^1';
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(3).file_name = '[Sess_1]_[Cond_1]_[cuexbackgroundEffort^1]';

    % Third modulator for first condition:
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(4).sess   = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(4).number = 1;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(4).pmod = 4;
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(4).name = 'cuexunsigned_effortPE';
    tmfc.ROI_set(ROI_set_number).gPPI.conditions(4).file_name = '[Sess_1]_[Cond_1]_[cuexunsigned_effortPE]';
...
if plot_only == 1
    load(fullfile(tmfc.project_path,'TMFC_AET.mat'),'tmfc');
    tmfc.project_path = '/Volumes/appsmaj-effort-prey-fmri-scholey/aet_fMRI/gPPI_analysis'; % reset the path since PPI was run on BEAR and we're plotting locally 
end

ROI_labels = {tmfc.ROI_set(ROI_set_number).ROIs.name}; 
nROIs = numel(ROI_labels);  % or size(conval_1,1)

close all
%% 'Value' contrast (contrast #2)
clear conval_1 thresholded_1
M(numel(subj)).paths = [];
for iSub = 1:numel(subj)
    ...
    file = fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI','ROI_to_ROI','symmetrical', ...
        ['Subject_00', id, '_Contrast_0002_[Sess_1]_[Cond_1]_[cuexvalue^1].mat']);
    S  = load(file); fn = fieldnames(S); M(iSub).paths = S.(fn{1});
end
clear S fn file
matrices = cat(3, M(:).paths);
...
% Perform one-sample t-test (two-sided, FDR-correction)
alpha = 0.001;                    % alpha = 0.001 threshold, corrected for two-sided comparison
correction = 'uncorr';                 % False Discovery Rate (FDR) correction (Benjamini–Hochberg procedure)

contrast = 1;                       % positive effect
[thresholded_1,pval,tval,conval_1] = tmfc_ttest(matrices,contrast,alpha,correction);
contrast = -1;                      % negative effect
[thresholded_2] = tmfc_ttest(matrices,contrast,alpha,correction);
...
subplot(1,3,1); imagesc(conval_1);        title('Group mean'); axis square; colorbar; caxis(tmfc_axis(conval_1,1));
subplot(1,3,2); imagesc(thresholded_1);   title('Positive (p uncorr < .001)'); axis square; colorbar;
subplot(1,3,3); imagesc(thresholded_2);   title('Negative (p uncorr < .001)'); axis square; colorbar;
colormap(subplot(1,3,2),'parula')
xticks(1:nROIs); xticklabels(ROI_labels);
yticks(1:nROIs); yticklabels(ROI_labels);
...
%% 'Background effort' contrast (contrast #3)
clear conval_1 thresholded_1
M(numel(subj)).paths = [];
for iSub = 1:numel(subj)
    id = num2str(iSub, '%02d');
    file = fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI','ROI_to_ROI','symmetrical', ...
        ['Subject_00', id, '_Contrast_0003_[Sess_1]_[Cond_1]_[cuexbackgroundEffort^1].mat']);
    S  = load(file); fn = fieldnames(S); M(iSub).paths = S.(fn{1});
end
clear S fn file
matrices = cat(3, M(:).paths);
...
%% 'Unsigned Effort PE' contrast (contrast #4)
clear conval_1 thresholded_1
M(numel(subj)).paths = [];
for iSub = 1:numel(subj)
    id = num2str(iSub, '%02d');
    file = fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI','ROI_to_ROI','symmetrical', ...
        ['Subject_00', id, '_Contrast_0004_[Sess_1]_[Cond_1]_[cuexunsigned_effortPE].mat']);
    S  = load(file); fn = fieldnames(S); M(iSub).paths = S.(fn{1});
end
clear S fn file
matrices = cat(3, M(:).paths);
