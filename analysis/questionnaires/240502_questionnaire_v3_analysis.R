...
filename = 'behav_summary_v3.csv' # behavioural study v1

#read data
d <- read.csv(paste('../../data_derived/v3/',filename, sep = ""))


# exclude subjects 1-16 for v3 of study (MVC not calibrated correctly, and changed instructions after that)
  d <- filter(d,subjectNumber > 16)
...
d_m_subj <- unique(d$subjectNumber)

#------------------------- read questionnaire data -------------------------- <- <- #
#read data
q <- readxl::read_xlsx('../../data_raw/v3/AET_questionnaire_results_v3.xlsx')
...
#read data
m <- read.csv(paste('../../data_derived/v3/fitting/',filename, sep = ""))
m$subjectNumber <- d_m_subj
m <- filter(m, subjectNumber %in% q$subjectNumber) # make sure we have the same subjects in each - only needed for v3, since not everyone did the questionnaires

m <- cbind(m,q$AMI_sum,q$AMI_ES_sum,q$AMI_SM_sum,q$AMI_BA_sum, q.accept.rate$Opportunity_cost_effect)


colnames(m) <- c('k', 'beta', 'alpha', 'fatigue', 'sub', 'AMI_Total', 'AMI_ES', 'AMI_SM', 'AMI_BA', 'opp_cost_effect')
